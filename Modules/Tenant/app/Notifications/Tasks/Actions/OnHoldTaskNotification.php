...
class OnHoldTaskNotification extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail($notifiable): MailMessage
    {
        return (new MailMessage)
            ->subject(
                "Task On Hold - {$this->title_en}"
            )
            ->greeting(
                "Hello " . ($notifiable->name ?? '') . ","
            )
            ->line(
                "A task has been successfully put on hold in {$this->company_name_en}."
            )
            ->line(
                "Task: {$this->title_en}"
            )
